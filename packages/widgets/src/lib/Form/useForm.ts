...
import { FormItemInstanceType } from './FormItem';

export interface FormInstanceType {
    wire: (
        name: string,
        _this: React.MutableRefObject<FormItemInstanceType>,
    ) => void;
    ...
    validates: (
        callback: (errors: any, value: any) => void,
        names?: string[],
    ) => void;
}
...
class FormInstance implements FormInstanceType {
    ...
    private wired: {
        [name: string]: React.MutableRefObject<FormItemInstanceType>;
    };
    constructor() {
    ...
    wire: FormInstanceType['wire'] = (name, _this) => {
        this.wired[name] = _this;
    };

    unWire: FormInstanceType['unWire'] = name => {
    ...
    getFieldsValue: FormInstanceType['getFieldsValue'] = names => {
        if (names) {
    ...
    getFieldValue: FormInstanceType['getFieldValue'] = name => {
        return this.store[name];
    };

    setFieldValue: FormInstanceType['setFieldValue'] = (name, value) => {
    ...
    setFieldsValue: FormInstanceType['setFieldsValue'] = values => {
        const oldStore = { ...this.store };
    ...
    resetFields: FormInstanceType['resetFields'] = () => {
        const oldStore = { ...this.store };
    ...
    scrollToField: FormInstanceType['scrollToField'] = name => {
        const _this = this.wired[name];
        if (!_this) return;
        _this.current.element.scrollIntoView();
    };

    validates: FormInstanceType['validates'] = (cb, names) => {
        let errors: any = null;
        let values = {};
